...
class _CancelFineScreenState extends ConsumerState<CancelFineScreen> {
  ...
  // Los países ahora vienen de Supabase a través del provider
    ...
    return Scaffold(
      ...
      body: Consumer(
        builder: (context, ref, child) {
          // Obtener países desde Supabase
          final countriesAsync = ref.watch(supportedCountriesProvider);
          final countries = countriesAsync.when(
            data: (data) => data,
  ...
  Widget _buildPlateInput(List<Map<String, dynamic>> countries) {
    return Padding(
  ...
  Future<void> _searchFine() async {
    ...
    try {
      // Buscar denuncias reales en Supabase usando el provider
      final fines = await ref.read(finesProvider(_plateText).future);

      if (fines.isNotEmpty) {
        // Mostrar la primera denuncia encontrada
        setState(() {
          _isSearching = false;
          _foundFine = fines.first;
        });
      } else {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('No se encontraron denuncias para esta matrícula'),
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Error al buscar denuncias: $e'),
          backgroundColor: Colors.red,
        ),
      );
    }
